...
double HCModelBasedRecombinationCalculator::ComputeAssemblyProbability(const HCRecombination& recombination) const {

    /** Probabilities of Gene Ids are computed */
    double V_gene_prob_id = model_.GetProbabilityByGenId(variable_gene, recombination.V().GeneId());
    V_gene_prob_id = log(V_gene_prob_id);

    double D_gene_prob_id = model_.GetProbabilityByGenId(diversity_gene, recombination.D().GeneId());
    D_gene_prob_id = log(D_gene_prob_id);

    double J_gene_prob_id = model_.GetProbabilityByGenId(join_gene, recombination.J().GeneId());
