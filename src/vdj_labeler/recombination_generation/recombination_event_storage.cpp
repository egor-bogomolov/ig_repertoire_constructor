...
bool IgGeneRecombinationEventStorage::CheckConsistency(CleavedIgGeneAlignment new_gene_event) {
...
void IgGeneRecombinationEventStorage::AddEvent(CleavedIgGeneAlignment new_gene_event) {
    ...
    if(left_position_map_.find(left_position) == left_position_map_.end())
        left_position_map_[left_position] = Range(recombination_events_.size() - 1,
                                                                           size_t(-1));
