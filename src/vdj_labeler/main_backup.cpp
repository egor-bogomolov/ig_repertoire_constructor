...
int main(int, char**) {
    ...
    {
        std::ifstream in("src/vdj_labeler/test_model.csv");
        HCProbabilityRecombinationModel model = HCProbabilityRecombinationModel(in, hc_db);
        //cout << model;
        ...
        cout << recombination_calculator.ComputeAssemblyProbability(*(recombination_storage.cbegin())) << endl;
        for(auto it = recombination_storage.cbegin(); it != recombination_storage.cend(); ++it)
            cout << recombination_calculator.ComputeAssemblyProbability(*it) << endl;
    }
